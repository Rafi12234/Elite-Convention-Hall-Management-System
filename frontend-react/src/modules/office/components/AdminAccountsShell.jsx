...
const SECTION_TABS = [
  { to: "/office/admin/accounts/money-in", label: "Money In", icon: Banknote },
  { to: "/office/admin/accounts/employees", label: "Employees", icon: UsersRound },
  { to: "/office/admin/accounts/bills", label: "Bills", icon: FileText },
  { to: "/office/admin/accounts/expenses", label: "Expenses", icon: ClipboardList },
  { to: "/office/admin/accounts/vendors", label: "Vendors", icon: Store },
];
...
export default function AdminAccountsShell({ title, subtitle, actions, children }) {
  ...
  useEffect(() => {
    fetchAdminMe()
      .then((me) => {
        if (!me) return navigate("/office/admin/login", { replace: true });
        setAdmin(me);
      })
  ...
  async function handleLogout() {
    await adminLogout();
    navigate("/office/admin/login", { replace: true });
  }
          ...
          >
            {({ isActive }) => (
              <>
                ...
                />
                {label}
                {to === "/office/admin/accounts/bills" && pendingBillsTotal > 0 ? (
                  <span
