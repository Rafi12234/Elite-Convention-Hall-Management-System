...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    fetchAdminMe()
      .then((me) => {
        if (me) navigate("/office/admin-dashboard", { replace: true });
      })
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate("/office/admin-dashboard", { replace: true });
    } catch (err) {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#fff9fc] px-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-mme-purple/55">Elite Convention Hall — Office Management</p>
        </div>
        ...
        <p className="mt-6 text-center text-xs text-mme-purple/40">
          <Link to="/office" className="hover:text-mme-purple">← Back to app</Link>
        </p>
      </div>
    </div>
  );
