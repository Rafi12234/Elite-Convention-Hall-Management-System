...
export default function AdminMeetingDetailsPage() {
  ...
  const backTo = location.state?.from || "/office/admin/activity";
  ...
  useEffect(() => {
    fetchAdminMe()
      .then((me) => {
        if (!me) return navigate("/office/admin", { replace: true });
        setAdmin(me);
      })
  ...
  async function handleLogout() {
    await adminLogout();
    navigate("/office/admin", { replace: true });
  }
        ...
        <div className="mx-auto flex max-w-350 items-center justify-between px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-mme-plum sm:text-xs">Elite Convention Hall</p>
            </div>
          </div>
          <button
